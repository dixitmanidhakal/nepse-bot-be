...
Ported from ``quant_pro.signal_ranking``. The original depended on
``event_layer.EventAdjustmentContext`` for optional news-driven score
adjustments; nepse-bot ships a minimal stub so callers who don't pass
``event_context`` get the default zero-adjustment behaviour without
pulling in the full news/OSINT pipeline.
"""
...
class EventAdjustmentContext:
    """
    Minimal stub of ``quant_pro.event_layer.EventAdjustmentContext``.

    When the caller supplies no context, the ranker treats all events as
    zero-adjustment. The full news-driven context lives in the quant-
    terminal and is not part of nepse-bot's surface area (yet).
    """
    run_date: str = ""
    ...
    def details_for(
        ...
        """Return zero-adjustment details — stub behaviour only."""
        return {
